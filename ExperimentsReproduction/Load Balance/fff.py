...
class FFF(nn.Module):
    ...
    def __init__(self,
            ...
            region_leak: float=0.0, usage_mode: str = 'none', new_impl=False, leaf_test = False):
        """
    ...
    def eval_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not self.new_impl:
            print("Old implementation not supported")
            return
        else:
        ...
        if (self.leaf_test == False):
            leaves = []
        return new_logits.view(*original_shape[:-1], self.output_width) , leaves	# (..., self.output_width)
