...
class FFF(nn.Module):
    ...
    def __init__(self,
        ...
        #magic code
        self.magic_w1= nn.Parameter(torch.empty((1, input_width, 8), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
        self.magic_b1 = nn.Parameter(torch.empty((1, 8), dtype=torch.float).uniform_(-l1_init_factor, +l1_init_factor), requires_grad=True)
        self.magic_w2 = nn.Parameter(torch.empty((1, 8, output_width), dtype=torch.float).uniform_(-l2_init_factor, +l2_init_factor), requires_grad=True)
        self.magic_b2 = nn.Parameter(torch.empty((1, output_width), dtype=torch.float).uniform_(-l2_init_factor, +l2_init_factor), requires_grad=True)
        ...
        if usage_mode != 'none':
            ...
            self.f = nn.Parameter(torch.zeros((self.n_leaves,), dtype=torch.float), requires_grad=False)
    ...
    def training_forward(self, x: torch.Tensor, return_entropies: bool=False, use_hard_decisions: bool=False):
        ...
        current_mixture_hard = torch.ones((batch_size, self.n_leaves), dtype=torch.float, device=x.device)
        entropies = None if not return_entropies else torch.zeros((batch_size, self.n_nodes), dtype=torch.float, device=x.device)
        l = torch.ones((batch_size, self.n_leaves), dtype=torch.float, device=x.device)
        f = torch.ones((batch_size, self.n_leaves), dtype=torch.float, device=x.device)

        if self.usage_mode != 'none' and self.depth.item() > 0:
            self.node_usage.data[0] = batch_size
        ...
        for current_depth in range(self.depth.item()):
            ...
            for i in range(n_nodes):
              counter = 0
              for j in range(int(self.n_leaves/n_nodes)):
                if(counter < self.n_leaves/(2*n_nodes) ):
                  l[:,j+i*int(self.n_leaves/n_nodes)]*=not_boundary_effect[:,i]
                  f[:,j+i*int(self.n_leaves/n_nodes)]*=(not_boundary_effect[:,i] >= 0.5).float()
                  counter +=1
                else:
                  l[:,j+i*int(self.n_leaves/n_nodes)]*=boundary_effect[:,i]
                  f[:,j+i*int(self.n_leaves/n_nodes)]*=(boundary_effect[:,i] >= 0.5).float()
                  counter +=1

            if True:
                boundary_effect_hard = torch.round(boundary_effect)				# (batch_size, n_nodes)
                not_boundary_effect_hard = 1 - boundary_effect_hard					# (batch_size, n_nodes)
            ...
            mixture_modifier_hard = torch.cat( # this cat-fu is to interleavingly combine the two tensors
                (not_boundary_effect_hard.unsqueeze(-1), boundary_effect_hard.unsqueeze(-1)),
                dim=-1
            ).flatten(start_dim=-2, end_dim=-1).unsqueeze(-1)												# (batch_size, n_nodes*2, 1)
            current_mixture_hard = current_mixture_hard.view(batch_size, 2 * n_nodes, self.n_leaves // (2 * n_nodes))	# (batch_size, 2*n_nodes, self.n_leaves // (2*n_nodes))
            current_mixture_hard.mul_(mixture_modifier_hard)															# (batch_size, 2*n_nodes, self.n_leaves // (2*n_nodes))
            current_mixture_hard = current_mixture_hard.flatten(start_dim=1, end_dim=2)
            ...
            if self.usage_mode != 'none' and current_depth != self.depth.item() - 1:
                if self.usage_mode == 'soft':
                    ...
                    current_node_usage = torch.round(mixture_modifier_hard).squeeze(-1).sum(dim=0)				# (n_nodes*2,)
                self.node_usage.data[next_platform:next_platform+n_nodes*2] = current_node_usage.detach()		# (n_nodes*2,)
                #self.ff[next_platform:next_platform+n_nodes*2] = torch.round(mixture_modifier_hard).squeeze(-1).sum(dim=0).detach()

            del mixture_modifier, boundary_effect, not_boundary_effect,mixture_modifier_hard, boundary_effect_hard, not_boundary_effect_hard, boundary_plane_logits, boundary_plane_coeff_scores, current_weights, current_biases
        ...
        if self.usage_mode != 'none':
            ...
            self.f.data = torch.round(current_mixture_hard).sum(dim=0).detach().to(device)
        #print(self.leaf_usage)
        #print(self.f)
        ...
        new_logits = torch.einsum('...li,lij->...lj', element_activations, self.w2s) + self.b2s	# (batch_size, self.n_leaves, self.output_width')
        # new_logits has shape (batch_size, self.n_leaves, self.output_width)

        # the magic ones
        magic_element_logits = torch.matmul(x, self.magic_w1.transpose(0, 1).flatten(1, 2))			# (batch_size, 1 * 8)
        magic_element_logits = magic_element_logits.view(batch_size, 1, 8)	# (batch_size, 1, 8)
        magic_element_logits += self.magic_b1.view(1, *self.magic_b1.shape)									# (batch_size, 1, 8)
        magic_element_activations = self.activation(magic_element_logits)								# (batch_size, 1, 8)
        magic_element_activations = self.leaf_dropout(magic_element_activations)						# (batch_size, 1, 8)
        magic_new_logits = torch.einsum('...li,lij->...lj', magic_element_activations, self.magic_w2) + self.magic_b2	# (batch_size, 1, self.output_width')
        magic_new_logits = magic_new_logits.sum(dim=1)      # (batch_size, self.output_width)
        ...
        final_logits = k*final_logits + (1-k)*magic_new_logits
        ...
        if not return_entropies:
            ...
            return final_logits, entropies.mean(dim=0), torch.mul(torch.sum(l,0),torch.sum(f,0)).sum()
    ...
    def eval_forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #return leaves
          		# (batch_size,)
        logits = torch.einsum('...i,...ij->...j', x, self.w1s[leaves]) + self.b1s[leaves]
        activations = self.activation(logits)
        new_logits = torch.einsum('...i,...ij->...j', activations, self.w2s[leaves]) + self.b2s[leaves]
        # magic code
        magic_logits = torch.einsum('...i,...ij->...j', x, self.magic_w1) + self.magic_b1
        magic_activations = self.activation(magic_logits)
        magic_new_logits = torch.einsum('...i,...ij->...j', magic_activations, self.magic_w2) + self.magic_b2

        k = torch.sigmoid(self.k)
        final_logits = k*new_logits + (1-k)*magic_new_logits

        #print(leaves)
        #print(predicted_test)

        return final_logits.view(*original_shape[:-1], self.output_width)	# (..., self.output_width)
